...
DOMAIN_FILE = REPO_DIR / "DOMAIN.txt"
...
def date_labels(value: Any) -> tuple[str, str]:
    ...
    return iso, f"{MONTHS_EN[parsed.month]} {parsed.day}, {parsed.year}"
...
def estimate_read_time(content_html: str) -> int:
    words = re.findall(r"\b[\w-]+\b", re.sub(r"<[^>]+>", " ", content_html))
    return max(3, round(len(words) / 200))
...
def render_post(data: dict[str, Any]) -> Path:
    ...
    date_iso, date_en = date_labels(data.get("date_iso"))
    ...
    return f'''      <a href="/blog/posts/{slug}.html" class="blog-card animate-up">
        ...
        <div class="blog-card-body">
          <div class="blog-card-meta">
            ...
            <time datetime="{date_iso}">{date_en}</time>
          </div>
...
def update_index(data: dict[str, Any]) -> None:
    ...
    marker = "      <!-- New posts are inserted here automatically -->"
...
def next_article_number(registry: str) -> int:
    match = re.search(r"- Total articles:\s*(\d+)", registry)
    if not match:
...
def update_registry(data: dict[str, Any]) -> None:
    ...
    registry = re.sub(r"- Total articles:\s*\d+", f"- Total articles: {number}", registry, count=1)
    registry = re.sub(r"- Last published:\s*\d{4}-\d{2}-\d{2}", f"- Last published: {date_iso}", registry, count=1)
    header = "|---|------|------|-------|----------|-----|---------|\n"
...
def main() -> None:
    ...
    content_html = as_text(data["content_html"], "content_html")
    if len(re.findall(r"\b[\w-]+\b", re.sub(r"<[^>]+>", " ", content_html))) < 700:
